/*
[2.0]
	Оптимизированы работы.
[2.0]
[2.1]
	Добавлены бутылки. При каждом действии,связынным с бомжатничеством вы будете подбирать рандомное количество бутылок.
	Бутылки можно сдавать. При сдаче одной бутылки вы получите 4 рубля. (Пункт в меню = 9)
[2.1]
[2.2]
	Добавлен перевод денег. Теперь доступны доллары. Для перевода выберите определенное количество денег. 1 доллар = 4 рубля.
	Разумеется, дейстсвует в обратную сторону.
[2.2]
[2.3]
	Добавлены новая работа - Махинации с Акциями. Изменены цены на некоторую недвижимость. Изменены цены на транспорт.
[2.3]
[2.4]
	Добавлен датчик контроля. Теперь при достижении низких показателей баров вас будут уведомлять о ухудшении состоянии персонажа и о нужных действиях.
[2.4]
[2.5]
	Обновлен статус на каждом ходе. При каждом ходе обновляется меню выбора ( для видимости клавиш выбора ).
[2.5]
*/
...
long test = 0;
long test2 = 0;
void translatemoney(long *a, long *b, long *c)
{
...
void translatedollars(long *a, long *b, long *c)
{
	*a += *b / 4;
	*c -= *b;
}
...
void Warning(int g, int he, int ha)
{
	if (g <= 10 && g >= 5)
	{
		cout << "Вы голодны! Вы умрете, если не поедите.\n";
	}
	if (he <= 10 && he >= 5)
	{
		cout << "Вы больны! Вы умрете, если не вылечитесь.\n";
	}
	if (ha <= 10 && ha >= 5)
	{
		cout << "Вы несчастны! Вы умрете, если не повеселитесь.\n";
	}
}
...
int main()
{
	...
	while (quit)
	{
		cout << "1.Информация" << endl << "2.Еда и настроение" << endl << "3.Здоровье и спорт" << endl << "4.Бомжатничать и поборы" << endl << "5.Работа" << endl << "6.Учеба" << endl << "7.Недвижимость и транспорт" << endl << "8.Политика" << endl << "9.Перевод денег" << endl << "10.Выход" << endl;
		int choice = 0;
		cin >> choice;
		switch (choice)
		{
		...
		case 1:
		{
			...
			cout << "Бутылки = " << bottles << endl;
			cout << "Доллары = " << dollars << endl << endl;
		...
		case 7:
		{
			...
			while (switch7 == 0)
			{
				...
				case 3:
				{
					if (dollars >= 75000 && housestatus < 3)
					{
						housestatus = 3;
						cout << "Вы успешно купили квартиру!\n";
						break;
					}
					else if (dollars < 75000 || housestatus == 3)
					{
				...
				case 4:
				{
					if (dollars >= 100000 && housestatus < 4)
					{
						housestatus = 4;
						cout << "Вы успешно купили офис!\n";
						break;
					}
					else if (dollars < 100000 || housestatus == 4)
					{
				...
				case 7:
				{
					if (dollars >= 50000 && trans < 3)
					{
						trans = 3;
						cout << "Вы успешно купили БМВ\n";
						break;
					}
					else if (dollars < 50000 || trans == 3)
					{
				...
				case 8:
				{
					if (dollars >= 120000 && trans < 4)
					{
						trans = 4;
						cout << "Вы успешно купили вертолет\n";
						break;
					}
					else if (dollars < 120000 || trans == 4)
					{
		...
		case 9:
		{
			system("cls");
			switch10 = 0;
			cout << "1.Перевести бутылки в рубли\n2.Перевод долларов в рубли\n3.Перевод рублей в доллары\n4.Выход\n";
			while (switch10 == 0)
			{
				Warning(golod, health, happy);
				checkparameters(golod, health, happy);
				...
				case 2:
				{
					cout << "Выберите количество долларов для перевода\n";
					cout << "Количество доступных долларов = " << dollars << endl;
					cin >> test;
					if (test > dollars || test <= 0)
					{
						cout << "Количество денег указано неверно\n";
						continue;
					}
					else
					{
						translatemoney(&money, &test, &dollars);
						cout << "Настроение - " << happy << endl << "Голод - " << golod << endl << "Здоровье - " << health << endl << "Деньги - " << money << endl << "Бутылки - " << bottles << endl;
						cout << "Остаток баланса = " << dollars << endl;
						break;
					}
					break;
				}
				case 3:
				{
					cout << "Выберите количество рублей для перевода\n";
					cout << "Количество доступных рублей = " << money << endl;
					cin >> test2;
					if (test2 > money || test2 <= 0)
					{
						cout << "Количество денег указано неверно\n";
						continue;
					}
					...
					{
						translatedollars(&dollars, &test2, &money);
						cout << "Настроение - " << happy << endl << "Голод - " << golod << endl << "Здоровье - " << health << endl << "Деньги - " << money << endl << "Бутылки - " << bottles << endl;
						cout << "Текущие доллары = " << dollars << endl;
						break;
					}
					break;
				}
